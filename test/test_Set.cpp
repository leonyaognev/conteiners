...
TEST(SetTest, InsertSingleValue) {
  Set<int> s;
...
TEST(SetTest, InsertMultipleUniqueValues) {
  Set<int> s;
  s.insert(20);
  s.insert(10);
  s.insert(30);
...
TEST(SetTest, InsertDuplicateValue) {
  Set<int> s = {10, 20};
  auto res = s.insert(10);
...
TEST(SetTest, InsertStringValues) {
  Set<std::string> s;
  s.insert("apple");
  s.insert("banana");
  s.insert("cherry");
...
TEST(SetTest, InsertInitializerListBasic) {
  Set<int> s = {1, 2};
  s.insert({3, 4, 5});
  ASSERT_EQ(s.size(), 5);
  ASSERT_TRUE(s.find(5) != s.end());
...
TEST(SetTest, InsertInitializerListWithDuplicates) {
  Set<int> s = {10, 20};
...
TEST(SetTest, InsertEmptyInitializerList) {
  Set<int> s = {1};
  s.insert({});
  ASSERT_EQ(s.size(), 1);
}

TEST(SetTest, InsertInitializerListStrings) {
  Set<std::string> s;
...
TEST(SetTest, InsertInitializerListLarge) {
  Set<int> s;
...
TEST(SetTest, InsertRangeIterators) {
...
TEST(SetTest, InsertRangeSameSet) {
  Set<int> s = {1, 2, 3};
...
TEST(SetTest, FindNonExistingElement) {
  Set<int> s = {10, 20, 30};
  auto it = s.find(40);
  ASSERT_TRUE(it == s.end());
}
...
TEST(SetTest, CountExistingElement) {
  Set<int> s = {10, 20, 30};
  ASSERT_EQ(s.count(20), 1);
}

TEST(SetTest, CountNonExistingElement) {
  Set<int> s = {10, 20, 30};
  ASSERT_EQ(s.count(40), 0);
}

TEST(SetTest, CountInEmptySet) {
  Set<int> s;
  ASSERT_EQ(s.count(10), 0);
}

TEST(SetTest, CountStringValue) {
  Set<std::string> s = {"a", "b", "c"};
  ASSERT_EQ(s.count("b"), 1);
  ASSERT_EQ(s.count("d"), 0);
}

TEST(SetTest, CountLargeSet) {
  Set<int> s;
...
TEST(SetTest, EraseByIteratorExisting) {
  Set<int> s = {10, 20, 30};
  auto it = s.find(20);
...
TEST(SetTest, EraseByIteratorRange) {
  Set<int> s;
  s.insert({1, 2, 3, 4, 5});
...
TEST(SetTest, EraseBeginIterator) {
  Set<int> s = {10, 20, 30};
  s.erase(s.begin());
  ASSERT_EQ(s.size(), 2);
  ASSERT_EQ(s.count(10), 0);
...
TEST(SetTest, EraseEndIteratorInvalid) {
  Set<int> s = {10};
...
TEST(SetTest, MergeEmptyIntoNonEmpty) {
  Set<int> s1 = {1, 2};
  Set<int> s2;
...
TEST(SetTest, ContainsInEmptySet) {
  Set<int> s;
  bool it = s.contains(10);
  ASSERT_FALSE(it);
}

TEST(SetTest, ContainsEdgeCases) {
  Set<int> s = {1, 100};
...
TEST(SetTest, ContainsStringValue) {
  Set<std::string> s = {"x", "y", "z"};
