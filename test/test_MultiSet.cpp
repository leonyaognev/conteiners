...
TEST(MultiSetTest, FindElements) {
  MultiSet<int> s;
  s.insert(10);
  s.insert(20);
  s.insert(10);
...
TEST(MultiSetTest, RangeConstructor) {
...
TEST(MultiSetTest, InsertSingleValue) {
  MultiSet<int> s;
  auto it = s.insert(10);
  ASSERT_EQ(*it, 10);
  ASSERT_EQ(s.size(), 1u);
...
TEST(MultiSetTest, InsertInitializerListWithDuplicates) {
...
TEST(MultiSetTest, InsertRangeWithDuplicates) {
...
TEST(MultiSetTest, EraseBeginIterator) {
...
TEST(MultiSetTest, EraseLargeMultiSet) {
  MultiSet<int> s;
  for (int i = 0; i < 100; i++) {
    s.insert(i);
    s.insert(i);
